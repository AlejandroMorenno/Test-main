<?php
   ...
   //  }
	...
	$where = "";
	if ($_POST["term"]) {
		$where = addWhere($where, "`term` = '" . htmlspecialchars($_POST["term"])) . "'";
	}
	if ($_POST["residential_object"]) {
		$where = addWhere($where, "`residential_object` = '" . htmlspecialchars($_POST["residential_object"])) . "'";
	}
	if ($_POST["bathrom"]) {
		$where = addWhere($where, "`bathrom` = '" . htmlspecialchars($_POST["bathrom"])) . "'";
	}
	if ($_POST["heating"]) {
		$where = addWhere($where, "`heating` = '" . htmlspecialchars($_POST["heating"])) . "'";
	}
	if ($_POST["number_of_rooms"]) {
		$where = addWhere($where, "`number_of_rooms` = '" . htmlspecialchars($_POST["number_of_rooms"])) . "'";
	}
	if ($_POST["low_cost"]) {
		$where = addWhere($where, "`cost` >= '" . htmlspecialchars($_POST["low_cost"])) . "'";
	}
	if ($_POST["hight_cost"]) {
		$where = addWhere($where, "`cost` <= '" . htmlspecialchars($_POST["hight_cost"])) . "'";
	}
	...
	// echo $sql;
	$apartment=mysqli_query($link,$sql);
...
<body>
    ...
    <form action="filters2.php" method="post">
            <div class="filters">
                ...
                <select name="residential_object">
        ...
        <div class="apartments">
            <?
				...
				while ($var = mysqli_fetch_assoc($apartment)){
						// if ($city_name !== '') {
						// 	$location = mysqli_query($link,"SELECT * FROM `location` WHERE `city` = '$city_name'");
						// }
						// if ($city_name == '') {
						// 	$location = mysqli_query($link,"SELECT * FROM `location`");
						// }
                    ...
                    if ($city == $city_name && $var["status"] == "Свободно") {
                        echo '
                        <div class = "item">
                            <div>
